...
namespace MeuCorre.Infra.Repositories
{
    public class TagRepository : ITagRepository
    {
        ...
        public async Task<bool> ExisteAsync(Guid tagId)
        {
            var existe = await _meuDbContext.Tags.AnyAsync(t => t.Id == tagId);
            return existe;
        ...
        {
            var listaTags = _meuDbContext.Tags
                .Where(t => t.UsuarioId == usuarioId);
            return await listaTags.ToListAsync();
        }

        public async Task<bool> NomeExisteParaUsuarioAsync(string nome, Guid usuarioId)
        {
